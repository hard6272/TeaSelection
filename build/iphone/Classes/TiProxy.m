...
//Rare exceptions to indicate a bug in the _teaselection code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_teaselection = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_teaselection]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.teaselection.module"
